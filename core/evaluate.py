...
def validate(args, model, data_loader,  bar):
    ...
    with torch.no_grad():
        for sampled_batch in data_loader:
            bar.move()
            image_batch, clinic_batch, radio_batch, labels = sampled_batch['image'],sampled_batch['clinic'],sampled_batch['radio'], sampled_batch['label']
            image_batch, clinic_batch, radio_batch, labels = image_batch.cuda(), clinic_batch.cuda(), radio_batch.cuda(),labels.cuda()


            preds = model([image_batch,clinic_batch,radio_batch])
        ...
        save_path='%s/%d' %(args.snapshot_path,args.epoch_num)
        if not os.path.exists(save_path):
            os.mkdir(save_path)
        path1='%s/pred_%s.csv' %(save_path,args.val_mode)
        path2='%s/label_%s.csv' %(save_path,args.val_mode)
        analysis_file='%s/metrics_%s.csv' %(save_path,args.val_mode)
        if not os.path.exists(analysis_file):
            metrics_id=open(analysis_file,'w')
            metrics_id.write('epoch_num,acc,sensitivity,specificity,precision,F1,auc,kappa,TN,FN,TP,FP\n')
        else:
            metrics_id=open(analysis_file,'a')
        mis_file='%s/wrongPred_%s.csv' %(save_path,args.val_mode)
        right_file='%s/rightPred_%s.csv' %(save_path,args.val_mode)
        results_single=np.argmax(results_arr,axis=1)
        labels_single=np.argmax(labels_arr,axis=1)
        w_id=open(mis_file,'w')
        w_id.write('image name,label,prediction\n')
        w_id2=open(right_file,'w')
        w_id2.write('image name,label,prediction\n')
        for i in range(len(args.val_list)):
            record=args.val_list[i]
            elems=record.split(',')
            img_name=elems[1]
            result_ = results_single[i]
            label_=labels_single[i]
            if result_!=label_:
                w_id.write('%s,%f,%.4f\n' %(img_name,label_,results_arr[i][1]))
            else:
                w_id2.write('%s,%f,%.4f\n' %(img_name,label_,results_arr[i][1]))
        w_id.close()
        w_id2.close()
        ...
        metrics_id.write('%d,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%d,%d,%d,%d\n' %(args.epoch_num,acc, sensitivity, specificity, precision,  F1, auc, kappa, TN, FN, TP, FP))
        metrics_id.close()
